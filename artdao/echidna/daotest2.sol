...
contract ArtDAO {
    ...
    function mint() external {
        ...
        assert(tokenOwner[tokenId] == address(this));
        lastMintTime = block.timestamp;
    ...
    function _startAuction(uint256 tokenId) internal {
        ...
        assert(auction.endTime > auction.startTime);
    ...
    function settleAuction(uint256 tokenId) external {
        ...
        if (auction.highestBidder != address(0)) {
            ...
            assert(tokenOwner[tokenId] == auction.highestBidder);
    ...
    function resolveDispute(uint256 disputeId) external {
        ...
        for (uint256 i = 0; i < d.jurors.length; i++) {
            ...
            if (d.usedVotingPower[juror] > 0) {
                uint256 jurorbal = address(juror).balance;
                payable(juror).transfer(JURY_REWARD);
                assert(address(juror).balance >jurorbal);
            }
        }
